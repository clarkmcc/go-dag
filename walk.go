...
type Walker struct {
	...
	diagsMap       map[Vertex]diags.Diagnostics
...
func (w *Walker) Wait() diags.Diagnostics {
	// Wait for completion
	w.wait.Wait()
...
func (w *Walker) walkVertex(v Vertex, info *walkerVertex) {
	...
	if depsSuccess {
		...
		d = d.Append(errors.New("upstream dependencies failed"))
		upstreamFailed = true
	}
	...
	w.diagsLock.Lock()
	if w.diagsMap == nil {
		w.diagsMap = make(map[Vertex]diags.Diagnostics)
	}
	w.diagsMap[v] = d
